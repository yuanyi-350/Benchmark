...
def square_add_eq_one : (algorithm x y).1 ^ 2 + (algorithm x y).2 ^ 2 = 1 := sorry
...
/-
P7.7.2 Suppose $A$ and $B$ are in $\mathbb{R}^{n \times n}$.
Give an algorithm for computing orthogonal $Q$ and $Z$ such that $Q^T A Z$ is upper Hessenberg and $Z^T B Q$ is upper triangular.
-/
...
variable {m n p : ℕ} {A : Matrix (Fin m) (Fin n) ℝ} {B : Matrix (Fin p) (Fin n) ℝ}
...
variable {n : ℕ} {S : Matrix (Fin n) (Fin n) ℝ} {u : Fin n → ℝ}
